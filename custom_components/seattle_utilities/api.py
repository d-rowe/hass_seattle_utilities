...
class SeattleUtilities:
    ...
    async def __get_oracle_location(self):
        ...
        data = await self.api_wrapper("get", "https://myutilities.seattle.gov/rest/auth/ssologin", headers=headers,
    ...
    async def get_accounts(self):
        ...
        data = await self.api_wrapper("post", "https://myutilities.seattle.gov/rest/account/list", json=payload,
                                      headers=headers, allow_redirects=False)
    ...
    async def get_account_holders(self, company_code):
        ...
        data = await self.api_wrapper("post", "https://myutilities.seattle.gov/rest/account/list/some", json=payload,
                                      headers=headers, allow_redirects=False)
        return await data.json()
    ...
    async def get_bill_list(self, company_code, account):
        ...
        data = await self.api_wrapper("post", "https://myutilities.seattle.gov/rest/billing/comparison", json=payload,
                                      headers=headers, allow_redirects=False)
        return await data.json()
    ...
    async def get_daily_data(self, account, service_id, start, end, meter):
        ...
        data = await self.api_wrapper("post", "https://myutilities.seattle.gov/rest/usage/month", json=payload,
                                      headers=headers, allow_redirects=False)
        return await data.json()
    ...
    async def get_latest_data(self, who):
            ...
            for account in holders["account"]:
                ...
                for bill in bill_list["billList"]:
                    for meter in bill["meters"]:
                        if meter not in meter_metadata:
        ...
        for metadata in meter_metadata:
            meter = metadata["meter"]
            bill = metadata["bill"]
            account = metadata["account"]
            meter_metadata = await self.get_daily_data(account, bill["serviceId"], time_yesterday_string, time_now_string, meter)
            for day in meter_metadata["history"]:
    ...
    async def api_wrapper(self, method: str, url: str, json: dict = None, data: dict = None, headers: dict = None,
        ...
        if headers is None:
            headers = {}
        _LOGGER.debug(
            f"method: {method}, url: {url}, json: {json}, data: {data}, headers: {headers}, allow_redirects: {allow_redirects}")
